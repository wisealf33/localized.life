    ...
    <div className="calendar-account">
      ...
      {personFormOpen ? (
        <section className="calendar-inline-form" aria-labelledby="add-person-title">
          <div className="account-inline-heading"><div><p className="eyebrow">One shared Person record</p><h2 id="add-person-title">Add a new person</h2><p className="muted">Only the profile name is required. Every other detail can be added whenever it is available.</p></div><button className="icon-button" type="button" aria-label="Close person form" onClick={() => setPersonFormOpen(false)}><XCircle /></button></div>
          <form className="form" onSubmit={addPerson}>
            <fieldset className="calendar-address-fields"><legend>Person identity and contact</legend><label>Profile name<input name="displayName" required autoComplete="name" /><span className="field-note">The only required field.</span></label><div className="person-profile-grid person-profile-name-grid"><label>First name<input name="firstName" autoComplete="given-name" /></label><label>Middle name<input name="middleName" autoComplete="additional-name" /></label><label>Last name<input name="lastName" autoComplete="family-name" /></label></div><div className="grid two"><label>Preferred name<input name="preferredName" /></label><label>Phone<input name="phone" type="tel" autoComplete="tel" /></label><label>Email<input name="email" type="email" autoComplete="email" /></label><label>Secondary phone<input name="secondaryPhone" type="tel" /></label></div></fieldset>
